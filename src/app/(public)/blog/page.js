    ...
    <div style={{ padding: '0', backgroundColor: '#0a0a0a', minHeight: '100vh' }}>
      ...
      {mainPost.image_url ? (
        <div style={{ position: 'relative', width: '100%', height: '60vh', minHeight: '400px', backgroundColor: '#111' }}>
          <img src={mainPost.image_url} alt={mainPost.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '0 2rem', marginTop: mainPost.image_url ? '-100px' : '0', position: 'relative', zIndex: 10 }}>
        
        <article style={{ marginBottom: '6rem' }}>
          <header style={{ marginBottom: '4rem', textAlign: 'left' }}>
            <h1 style={{ fontSize: '4rem', color: '#fff', marginBottom: '1.5rem', lineHeight: '1.1', fontFamily: 'serif' }}>{mainPost.title}</h1>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #333', borderBottom: '1px solid #333', padding: '1rem 0' }}>
              <time style={{ color: '#888', fontSize: '1rem', textTransform: 'uppercase', letterSpacing: '1px' }}>
                {new Date(mainPost.created_at).toLocaleDateString('es-ES', { year: 'numeric', month: 'long', day: 'numeric' })}
              </time>
              ...
              <div style={{ display: 'flex', gap: '1rem' }}>
                ...
                {/* WhatsApp */}
                <a href={`https://api.whatsapp.com/send?text=${encodeURIComponent(mainPost.title)}%20https://lumina-photo-stock.vercel.app/blog/${mainPost.slug}`} target="_blank" rel="noopener noreferrer" style={{ color: '#25D366', textDecoration: 'none', padding: '0.5rem', backgroundColor: '#111', borderRadius: '50%', width: '35px', height: '35px', display: 'flex', justifyContent: 'center', alignItems: 'center', border: '1px solid #222' }} title="Compartir en WhatsApp">
                ...
                {/* Facebook */}
                <a href={`https://www.facebook.com/sharer/sharer.php?u=https://lumina-photo-stock.vercel.app/blog/${mainPost.slug}`} target="_blank" rel="noopener noreferrer" style={{ color: '#1877F2', textDecoration: 'none', padding: '0.5rem', backgroundColor: '#111', borderRadius: '50%', width: '35px', height: '35px', display: 'flex', justifyContent: 'center', alignItems: 'center', border: '1px solid #222' }} title="Compartir en Facebook">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-1.667c0-.955.192-1.333 1.115-1.333h2.885v-5h-3.808c-3.596 0-5.192 1.583-5.192 4.615v3.385z"/></svg>
                </a>

                {/* X (Twitter) */}
                <a href={`https://twitter.com/intent/tweet?url=https://lumina-photo-stock.vercel.app/blog/${mainPost.slug}&text=${encodeURIComponent(mainPost.title)}`} target="_blank" rel="noopener noreferrer" style={{ color: '#fff', textDecoration: 'none', padding: '0.5rem', backgroundColor: '#111', borderRadius: '50%', width: '35px', height: '35px', display: 'flex', justifyContent: 'center', alignItems: 'center', border: '1px solid #222' }} title="Compartir en X">
          ...
          <div className="blog-content">
            <ReactMarkdown>{mainPost.content}</ReactMarkdown>
          </div>
        </article>
        ...
        {otherPosts.length > 0 && (
          <div style={{ marginTop: '4rem', borderTop: '2px solid #222', paddingTop: '4rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '2rem' }}>
              {otherPosts.map(post => (
                <article key={post.id} style={{ gridColumn: 'span 4', display: 'flex', flexDirection: 'column', gap: '1rem', backgroundColor: '#111', padding: '1rem', borderRadius: '8px', border: '1px solid #222' }}>
                  {post.image_url && (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', flexGrow: 1 }}>
                    ...
                    {post.subtitle ? (
                      <p style={{ color: '#aaa', fontSize: '0.95rem', marginBottom: '1.5rem', lineHeight: '1.4' }}>{post.subtitle.substring(0, 100)}...</p>
                    ) : (
                      <p style={{ color: '#aaa', fontSize: '0.95rem', marginBottom: '1.5rem', lineHeight: '1.4' }}>{post.content.replace(/[#*]/g, '').substring(0, 100)}...</p>
                    )}
                    <Link href={`/blog/${post.slug}`} style={{ color: '#d4af37', textDecoration: 'none', fontWeight: 'bold', fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '1px', marginTop: 'auto' }}>
                      Leer Más →
                    </Link>
                  </div>
                </article>
